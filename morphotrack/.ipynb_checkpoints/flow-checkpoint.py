...
def dust_removal(binary,threshold=10):
    """
    The function removes the object smaller than threshold.
    Arguments:
        binary (ndarray):
        threshold (int): the number of pixel. The object smaller than the threshold will be removed.
    """
    label_objects, nb_labels = ndi.label(binary, structure=np.ones((3, 3, 3)))
    sizes = np.bincount(label_objects.ravel())
    sizes[0] = 0  
    sel = np.arange(sizes.size)[sizes > threshold]
    filtered_binary = np.isin(label_objects, sel)
...
def skeleton(binary, threshold=10):
    ...
    filtered_skeleton = dust_removal(skeletonized,threshold)
...
def get_neighbor_vectors(positions, reference_vectors, k=27, return_image=False):
    ...
    for i, pos in tqdm(enumerate(positions.tolist())):
