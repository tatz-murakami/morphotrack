...
def align_points_to_grid(coords, grid_interval=10, radius=None, get_largest_segment=True, *arg, **kwarg):
    """
    Move coordinates to grid. Only support 2D array.
        coords (ndarray): each row indicates the coordinate of a point.
        grid_interval (int): the interval of the sampling points
        radius (int): the radius used for morphological closing. If None, the grid_interval is used.
        get_largest_segment (bool): If true, remove the isolated points.
    """
    if radius is None:
        radius = grid_interval
        
    rounded_coords = np.round(coords).astype(int)
    translation = np.min(rounded_coords, axis=0)
    translated_rounded_coords = rounded_coords - translation
    upper_end = translated_rounded_coords.max(axis=0)+1
    
    image = np.zeros(upper_end)
    image[tuple(translated_rounded_coords.T)] = 1
    
    # morphological closing to fill holes
    closed_image = morphology.isotropic_closing(image, radius)
    
    # extract only largest segment
    if get_largest_segment:
        closed_image = morphology.remove_small_objects(closed_image, *arg, **kwarg)
        closed_image = ndimage.binary_fill_holes(closed_image)
    
    grids = np.mgrid[0:upper_end[0]:grid_interval, 0:upper_end[1]:grid_interval]
    grid_coords = np.stack([np.ravel(grids[0,...]),np.ravel(grids[1,...])])
    
    in_grid_coords = grid_coords.T[closed_image[tuple(grid_coords)]] + translation
    
    return in_grid_coords
